...
const ApiQuota = {
  ...
  async refresh(loud=false){if(this.busy)return; if(!Sync.on()){this.state('开启双端同步后可显示 Grok 周限额');return loud&&UI.toast('请先开启双端同步');}this.busy=true;this.state('正在读取 Grok 周限额…');try{const j=await Sync.call('quota');Store.set('_quotaCache',{weekly:j.grokWeekly||null,at:Date.now(),errors:{}});if(loud)UI.toast(j.grokWeekly?'Grok 周限额已更新':'尚未收到 Grok 使用量');}catch(e){Store.set('_quotaCache',{at:Date.now(),errors:{grok:e.message}});if(loud)UI.toast(e.message);}finally{this.busy=false;this.render();}},
  ...
  rowsHtml(k,c,d,m){
    ...
    const weekly=c.weekly,used=Math.max(0,Math.min(100,Number(weekly?.used)||0)),remaining=100-used,parts=(weekly?.breakdown||[]).map(x=>`${x.name} ${Number(x.used).toFixed(0)}%`).join(' · '),grokVal=weekly?`${remaining.toFixed(0)}%`:'—',grokSub=weekly?`已用 ${used.toFixed(0)}%${parts?` · ${parts}`:''}${weekly.resetAt?` · ${weekly.resetAt} 重置`:''}`:(c.errors?.grok||'请在 Grok 使用量页面打开同步扩展');
    return this.qrow(icWave,'Grok 每周剩余额度',grokVal,remaining,grokSub);
  },
  render(){
    ...
    const at=c.weekly?.updatedAt?new Date(c.weekly.updatedAt).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'}):'';this.state(at?`扩展上次同步：${at}。`:'' );
  },
